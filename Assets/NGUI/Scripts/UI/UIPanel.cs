...
#endif
		{
			...
			for (int i = 0, imax = mChildren.Count; i < imax; ++i)
			{
				...
				if (node.HasChanged())
				{
					...
					if (min == 0f)
					{
						Debug.LogWarning("Scale of 0 is invalid! Zero cannot be divided by, which causes problems. Use a small value instead, such as 0.01\n" +
                        node.trans.lossyScale + node.trans.gameObject.name, node.trans);
					}
#endif
				}
